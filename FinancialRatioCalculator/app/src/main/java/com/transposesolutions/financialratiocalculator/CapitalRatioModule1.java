...
public class CapitalRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_capital_ratio_module1);
        ...
        input3.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float total_liabilities = Float.parseFloat(input3.getText().toString());
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (total_liabilities < 100) {
                requiredCheck3();
            }else {

                // steps to calculate Return on Capital Employed Ratio
                double capital = (total_assets - total_liabilities);
                double roce = (earnings_tax / capital)*100;
                roce = Math.round(roce * 100.0)/100.0;
                System.out.println(roce);
                String _result =(roce + "%");
                // steps to display result details
                double result_detail = (earnings_tax / capital);
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail =   "Based on the above result,  return on capital employed is" + roce
                        + "%. It means that, company yields a return of $ " + result_detail + " per dollar for its capital employed." ;


                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(CapitalRatioModule1.this, CapitalRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(CapitalRatioModule1.this);
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, CapitalRatioModule1.class);
        startActivity(intent);
    }
}