...
public class InvestmentRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_investment_ratio_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float net_income = Float.parseFloat(input1.getText().toString());
                if (net_income < 100) {
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float shareholder_equity = Float.parseFloat(input2.getText().toString());
                if (shareholder_equity < 1) {
                    input2.setError("Please fill out this field! Enter a minimum value of 1 or greater value");
                }} catch (NumberFormatException e) {
                e.printStackTrace();
            }});
        ...
        input3.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float long_term_liabilities = Float.parseFloat(input3.getText().toString());
                if (long_term_liabilities < 1) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (shareholder_equity < 1) {
                requiredCheck2();
            }
            //check for required minimum value
            else if (long_term_liabilities < 1) {
                requiredCheck3();
            }else {

                // steps to calculate Return on Capital Employed Ratio
                // steps to calculate assets turnover in terms of times/multiples
                double coi = (shareholder_equity + long_term_liabilities);
                double roi = (net_income / coi) * 100;
                roi = Math.round(roi * 100.0)/100.0;
                System.out.println(roi);
                String _result =(roi + "%");
                // steps to display result details
                double result_detail = (net_income / coi);
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail =   "Based on the above result,  return on investment is " + roi
                        + "%. It means that, company yields a return of $ " + result_detail + " per dollar on its investments." ;


                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(InvestmentRatioModule1.this, InvestmentRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(InvestmentRatioModule1.this);
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(InvestmentRatioModule1.this);
        builder.setMessage("Shareholders equity value must be at least 1 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    private void requiredCheck3() {
        AlertDialog.Builder builder = new AlertDialog.Builder(InvestmentRatioModule1.this);
        builder.setMessage("Long-term liability value must be at least 1 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, InvestmentRatioModule1.class);
        startActivity(intent);
    }
}