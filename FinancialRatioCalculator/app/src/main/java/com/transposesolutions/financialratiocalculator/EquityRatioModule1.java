...
public class EquityRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_equity_ratio_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float total_equity = Float.parseFloat(input1.getText().toString());
                if (total_equity < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
                ...
                // steps to calculate equity ratio
                double equity_ratio = (total_equity / total_assets);
                equity_ratio = Math.round(equity_ratio * 100.0)/100.0;
                System.out.println(equity_ratio);
                String _result =(equity_ratio + " : 1");
                // steps to display result details
                double result_detail = (total_equity / total_assets);
                result_detail = Math.round(result_detail * 100.0);
                System.out.println(result_detail);
                String _detail =   "Based on the above result, equity ratio is "
                        + equity_ratio + " : 1 means that, " + result_detail + "%  of its assets are financed by equity." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(EquityRatioModule1.this, EquityRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(EquityRatioModule1.this);
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, EquityRatioModule1.class);
        startActivity(intent);
    }
}