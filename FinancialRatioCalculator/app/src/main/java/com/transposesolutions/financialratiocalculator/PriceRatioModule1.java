...
public class PriceRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
...
public final static String USER_INPUT_4 = "com.transposesolutions.financialratiocalculator.message_6";
...
public void afterTextChanged(Editable editable) {
        ...
        buttonCalculate.setEnabled(!s1.equals("")  && !s2.equals("")  && !s3.equals("") && !s4.equals(""));
        }
...
@Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_price_ratio_module1);
    ...
    input1.setOnFocusChangeListener((v, hasFocus) -> {
        try {
            ...
            float share_price = Float.parseFloat(input1.getText().toString());
            if (share_price < 1) {
                input1.setError("Please fill out this field! Enter a minimum value of 1 or greater value");
    ...
    input2.setOnFocusChangeListener((v, hasFocus) -> {
        try {
            // Retrieve user input Value from the activity
            float net_income = Float.parseFloat(input2.getText().toString());
            if (net_income < 100) {
    ...
    input3.setOnFocusChangeListener((v, hasFocus) -> {
        try {
            // Retrieve user input Value from the activity
            float preferred_dividends = Float.parseFloat(input3.getText().toString());
            if (preferred_dividends < 100) {
    ...
    input4.setOnFocusChangeListener((v, hasFocus) -> {
        try {
            // Retrieve user input Value from the activity
            float shares_outstanding = Float.parseFloat(input4.getText().toString());
            if (shares_outstanding < 100) {
                input4.setError("Please fill out this field! Enter a minimum value of 100 or greater value");
            }} catch (NumberFormatException e) {
            e.printStackTrace();
        }});
    ...
    buttonCalculate.setOnClickListener(view -> {
        ...
        //check for required minimum value
        else if (preferred_dividends < 100) {
            requiredCheck3();
        }
        //check for required minimum value
        else if (shares_outstanding < 100) {
            requiredCheck4();
        }else {
            // steps to calculate Price to Earnings
            double eps_ratio = (net_income-preferred_dividends)/shares_outstanding;
            double pe_ratio = (share_price / eps_ratio);
            pe_ratio = Math.round(pe_ratio * 100.0)/100.0;
            System.out.println(pe_ratio);
            String _result = (pe_ratio + " times");
            // steps to calculate result detail
            double result_detail = (share_price / eps_ratio);
            result_detail = Math.round(result_detail * 100.0)/100.0;
            System.out.println(result_detail);
            String _detail = "Based on the above result,  the company's price to earnings ratio is " + pe_ratio
                    + " times. It means that, the company stock is trading at a multiple of " + result_detail;

            // Method used to copy the above declared strings from this activity and forward it to another activity
            Intent intent = new Intent(PriceRatioModule1.this, PriceRatioModule2.class);
            ...
            String message_6 = input4.getText().toString();
            ...
            bun.putString(USER_INPUT_4, message_6);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(PriceRatioModule1.this);
        builder.setMessage("Share price value must be at least 1 or greater value")
                .setCancelable(false)
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(PriceRatioModule1.this);
        builder.setMessage("Net income value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    private void requiredCheck3() {
        AlertDialog.Builder builder = new AlertDialog.Builder(PriceRatioModule1.this);
        builder.setMessage("Preferred dividends value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    // check for required minimum value for user input4
    private void requiredCheck4() {
        AlertDialog.Builder builder = new AlertDialog.Builder(PriceRatioModule1.this);
        builder.setMessage("Shares outstanding value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, PriceRatioModule1.class);
        startActivity(intent);
    }
}