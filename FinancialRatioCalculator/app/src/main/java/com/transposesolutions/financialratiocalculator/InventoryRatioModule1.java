...
public class InventoryRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_inventory_ratio_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float COGS = Float.parseFloat(input1.getText().toString());
                if (COGS < 100) {
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float begin_inventory = Float.parseFloat(input2.getText().toString());
                if (begin_inventory < 100) {
        ...
        input3.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float end_inventory = Float.parseFloat(input3.getText().toString());
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (begin_inventory < 100) {
                requiredCheck2();
            }
            //check for required minimum value
            else if (end_inventory < 100) {
                requiredCheck3();
            }else {
                // steps to calculate inventory turnover ratio
                double average_inventory = (begin_inventory + end_inventory)/2;
                double inventory_turnover = (COGS / average_inventory);
                inventory_turnover = Math.round(inventory_turnover * 100.0)/100.0;
                System.out.println(inventory_turnover);
                String _result = (inventory_turnover + " times");
                // steps to calculate result detail
                double result_detail = (1/inventory_turnover) * 365;
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result,  inventory turnover ratio of " + inventory_turnover
                        + " times, means that the company sold its entire inventory within " + result_detail
                        + " day period." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(InventoryRatioModule1.this, InventoryRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(InventoryRatioModule1.this);
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(InventoryRatioModule1.this);
        builder.setMessage("Beginning inventory value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    private void requiredCheck3() {
        AlertDialog.Builder builder = new AlertDialog.Builder(InventoryRatioModule1.this);
        builder.setMessage("Ending inventory value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
