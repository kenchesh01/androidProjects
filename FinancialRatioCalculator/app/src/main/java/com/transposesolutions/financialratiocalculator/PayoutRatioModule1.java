...
public class PayoutRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_payout_ratio_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float total_dividends = Float.parseFloat(input1.getText().toString());
                if (total_dividends < 100) {
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float _netIncome = Float.parseFloat(input2.getText().toString());
                if (_netIncome < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
                ...
                // steps to calculate Dividend payout ratio
                double dividend_payout = total_dividends / net_income;
                dividend_payout = Math.round(dividend_payout * 100.0);
                System.out.println(dividend_payout);
                String _result = (dividend_payout + " %");
                // steps to display result details
                double result_detail = total_dividends / net_income;
                result_detail = Math.round(result_detail * 100.0);
                System.out.println(result_detail);
                String _detail =   "Based on the above result, the company is paying out "
                        + result_detail + "%  of its net income to its shareholders in the form of dividends." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(PayoutRatioModule1.this, PayoutRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(PayoutRatioModule1.this);
        builder.setMessage("Total dividends value must be at least 100 or greater value")
                .setCancelable(false)
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, PayoutRatioModule1.class);
        startActivity(intent);
    }
}