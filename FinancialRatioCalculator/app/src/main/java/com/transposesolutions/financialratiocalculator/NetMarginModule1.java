...
public class NetMarginModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_net_margin_module1);
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
                ...
                // steps to calculate Net Margin
                double net_margin = (net_income / earnings) * 100;
                net_margin = Math.round(net_margin * 100.0) / 100.0;
                System.out.println(net_margin);
                String _result = (net_margin + " %");
                // steps to calculate result detail
                double result_detail = (net_income / earnings) * 100;
                result_detail = Math.round((1 * result_detail / 100.0) * 100) / 100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result, the company has a net income of  $" + result_detail
                        + " for each dollar it generated or we can say that, company converted "
                        + net_margin + "% of its sales into profits.";

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(NetMarginModule1.this, NetMarginModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(NetMarginModule1.this);
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, NetMarginModule1.class);
        startActivity(intent);
    }
}