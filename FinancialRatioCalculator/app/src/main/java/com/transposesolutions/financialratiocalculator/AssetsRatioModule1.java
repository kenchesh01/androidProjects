...
public class AssetsRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_assets_ratio_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float net_equity = Float.parseFloat(input1.getText().toString());
                if (net_equity < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
                ...
                // steps to calculate Return on Assets Ratio
                double return_assets = (net_equity / total_assets) * 100;
                return_assets = Math.round(return_assets * 100.0)/100.0;
                System.out.println(return_assets);
                String _result = (return_assets + "%");
                // steps to calculate result detail
                double result_detail = (net_equity / total_assets) * 100;
                result_detail = Math.round((1 * result_detail / 100.0) * 100)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result, for every dollar that was invested in assets during the year produced $"
                        + result_detail + " of net income." ;
                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(AssetsRatioModule1.this, AssetsRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(AssetsRatioModule1.this);
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, AssetsRatioModule1.class);
        startActivity(intent);
    }
}