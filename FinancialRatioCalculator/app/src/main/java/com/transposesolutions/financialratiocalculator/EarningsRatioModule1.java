...
public class EarningsRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_earnings_ratio_module1);
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float preferred_dividends = Float.parseFloat(input2.getText().toString());
        ...
        input3.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float shares_outstanding = Float.parseFloat(input3.getText().toString());
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
                ...
                double retained_income = net_income - preferred_dividends;
                double eps_ratio = retained_income/shares_outstanding;
                eps_ratio = Math.round(eps_ratio * 100.0)/100.0;
                System.out.println(eps_ratio);
                String _result = String.valueOf(eps_ratio);
                // steps to calculate result detail
                double result_detail = retained_income/shares_outstanding;
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result,  the company's Earnings Per Share (EPS) for the year is, $"
                        + result_detail + " Per Share.";

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(EarningsRatioModule1.this, EarningsRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(EarningsRatioModule1.this);
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, EarningsRatioModule1.class);
        startActivity(intent);
    }
}