...
public class DebtEquityModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
...
public void afterTextChanged(Editable editable) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_debt_equity_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float current_liabilities = Float.parseFloat(input1.getText().toString());
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float equity = Float.parseFloat(input2.getText().toString());
                if (equity < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
                ...
                // steps to calculate Debt to Equity ratio
                double debt_equity = current_liabilities / equity;
                debt_equity = Math.round(debt_equity * 100.0)/100.0;
                System.out.println(debt_equity);
                String _result = (debt_equity + " : 1");
                // steps to display result details
                double result_detail = current_liabilities / equity;
                result_detail = Math.round(result_detail * 100.0);
                System.out.println(result_detail);
                String _detail =   "Based on the above result, the company has an exposure of "
                        + result_detail + "%  in the form of total liabilities.";

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(DebtEquityModule1.this, DebtEquityModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(DebtEquityModule1.this);
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(DebtEquityModule1.this);
        builder.setMessage("Total equity value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, DebtEquityModule1.class);
        startActivity(intent);
    }
}