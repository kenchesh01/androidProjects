...
public class ReturnRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_return_ratio_module1);
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float share_equity = Float.parseFloat(input2.getText().toString());
                if (share_equity < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (share_equity < 100) {
                requiredCheck2();
            } else {

                // steps to calculate Return on Equity Ratio
                float roe = (net_income / share_equity) * 100;
                roe = (float) (Math.round(roe * 100.0)/100.0);
                System.out.println(roe);
                String _result = (roe + "%");
                // steps to calculate result detail
                double result_detail = (net_income / share_equity) * 100;
                result_detail = Math.round((1 * result_detail / 100.0) * 100)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result, a ROE of " + roe + "% means that, $"
                        + result_detail + " of return earned on the shareholder's equity." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(ReturnRatioModule1.this, ReturnRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(ReturnRatioModule1.this);
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(ReturnRatioModule1.this);
        builder.setMessage("Shareholders Equity value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, ReturnRatioModule1.class);
        startActivity(intent);
    }
}