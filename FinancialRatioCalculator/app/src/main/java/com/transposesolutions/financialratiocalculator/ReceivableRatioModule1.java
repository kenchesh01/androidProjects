...
public class ReceivableRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_receivable_ratio_module1);
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float begin_accounts_receivable = Float.parseFloat(input2.getText().toString());
                if (begin_accounts_receivable < 100) {
        ...
        input3.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float end_accounts_receivable = Float.parseFloat(input3.getText().toString());
                if (end_accounts_receivable < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (begin_accounts_receivable < 100) {
                requiredCheck2();
            }
            //check for required minimum value
            else if (end_accounts_receivable < 100) {
                requiredCheck3();
            }else {
                // steps to calculate accounts receivable
                double average_accounts_receivable = (begin_accounts_receivable + end_accounts_receivable)/2;
                double accounts_receivable = (net_sales / average_accounts_receivable);
                accounts_receivable = Math.round(accounts_receivable * 100.0)/100.0;
                System.out.println(accounts_receivable);
                String _result  =(accounts_receivable + " times");
                // steps to calculate result detail
                double result_detail = (1/accounts_receivable) * 365;
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result,  account receivable turnover ratio of " + accounts_receivable
                        + " times, means an average customer took " + result_detail
                        + " days to pay the company's credit sales." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(ReceivableRatioModule1.this, ReceivableRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(ReceivableRatioModule1.this);
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(ReceivableRatioModule1.this);
        builder.setMessage("Beginning accounts receivable value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    private void requiredCheck3() {
        AlertDialog.Builder builder = new AlertDialog.Builder(ReceivableRatioModule1.this);
        builder.setMessage("Ending accounts receivable value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
