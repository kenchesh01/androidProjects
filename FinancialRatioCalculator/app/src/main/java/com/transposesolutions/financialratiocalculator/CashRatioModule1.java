...
public class CashRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cash_ratio_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float cash_equivalents = Float.parseFloat(input1.getText().toString());
                if (cash_equivalents < 100) {
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float marketable_securities = Float.parseFloat(input2.getText().toString());
                if (marketable_securities < 100) {
        ...
        input3.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float current_liabilities = Float.parseFloat(input3.getText().toString());
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (marketable_securities < 100) {
                requiredCheck2();
            }
            //check for required minimum value
                ...
                // steps to calculate cash ratio
                double liquid_assets = (cash_equivalents + marketable_securities);
                double cash_ratio = (liquid_assets / current_liabilities);
                cash_ratio = Math.round(cash_ratio * 100.0)/100.0;
                System.out.println(cash_ratio);
                String _result =(cash_ratio + " : 1");
                // steps to calculate result detail
                double result_detail = cash_ratio;
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result,  company has $" + cash_ratio
                        + " of liquid assets available to cover each $1 of short-term debt (current liabilities)." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(CashRatioModule1.this, CashRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(CashRatioModule1.this);
        builder.setMessage("Cash equivalents value must be at least 100 or greater value")
                .setCancelable(false)
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(CashRatioModule1.this);
        builder.setMessage("Market securities value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, CashRatioModule1.class);
        startActivity(intent);
    }
}