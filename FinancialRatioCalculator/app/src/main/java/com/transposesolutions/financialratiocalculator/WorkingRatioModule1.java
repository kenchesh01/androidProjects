...
public class WorkingRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_working_ratio_module1); // Load Navigation View, add toggle button to the drawer layout , setNavigationItemSelectedListener
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float current_assets = Float.parseFloat(input1.getText().toString());
                if (current_assets < 100) {
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float current_liabilities = Float.parseFloat(input2.getText().toString());
                if (current_liabilities < 100) {
        ...
        input3.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float total_assets = Float.parseFloat(input3.getText().toString());
                if (total_assets < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (current_liabilities < 100) {
                requiredCheck2();
            }
            //check for required minimum value
                ...
                // steps to calculate net working capital ratio
                double net_working_capital = current_assets - current_liabilities;
                double net_working_capital_ratio = (net_working_capital / total_assets);
                net_working_capital_ratio = Math.round(net_working_capital_ratio * 100.0)/100.0;
                System.out.println(net_working_capital_ratio);
                String _result = (net_working_capital_ratio + " : 1");
                // steps to calculate result detail
                double result_detail = (net_working_capital / total_assets);
                result_detail = Math.round(result_detail * 100);
                System.out.println(result_detail);
                String _detail = "Based on the above result, company has a Net working capital ratio of "
                        + net_working_capital_ratio +" : 1" + ". It means " + result_detail
                        + "% of totals assets is in the form of short-term assets (Net working capital – liquid funds)." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(WorkingRatioModule1.this, WorkingRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(WorkingRatioModule1.this);
        builder.setMessage("Current assets value must be at least 100 or greater value")
                .setCancelable(false)
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(WorkingRatioModule1.this);
        builder.setMessage("Current liabilities value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, WorkingRatioModule1.class);
        startActivity(intent);
    }
}