...
public class CoverageRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_coverage_ratio_module1);
        ...
        input1.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                ...
                float earnings_tax = Float.parseFloat(input1.getText().toString());
                if (earnings_tax < 100) {
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float interest_expense = Float.parseFloat(input2.getText().toString());
                if (interest_expense < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
            else if (interest_expense < 100) {
                requiredCheck2();
            } else {
                // steps to calculate interest coverage ratio
                double interest_coverage_ratio = earnings_tax / interest_expense;
                interest_coverage_ratio = Math.round(interest_coverage_ratio * 100.0)/100.0;
                System.out.println(interest_coverage_ratio);
                String _result = (interest_coverage_ratio + " : 1");
                // steps to calculate result detail
                double result_detail = earnings_tax / interest_expense;
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result, the company can meet its interest payments "
                        + result_detail + " times from its earnings.";

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(CoverageRatioModule1.this, CoverageRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(CoverageRatioModule1.this);
        builder.setMessage("Earnings before interest and tax value must be at least 100 or greater value")
                .setCancelable(false)
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(CoverageRatioModule1.this);
        builder.setMessage("Interest expense value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, CoverageRatioModule1.class);
        startActivity(intent);
    }
}