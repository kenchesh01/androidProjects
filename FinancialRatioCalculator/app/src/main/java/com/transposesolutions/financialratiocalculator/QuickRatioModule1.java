...
public class QuickRatioModule1 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quick_ratio_module1); // Load Navigation View, add toggle button to the drawer layout , setNavigationItemSelectedListener
        ...
        input2.setOnFocusChangeListener((v, hasFocus) -> {
            try {
                // Retrieve user input Value from the activity
                float inventory = Float.parseFloat(input2.getText().toString());
                if (inventory < 100) {
        ...
        buttonCalculate.setOnClickListener(view -> {
            ...
            //check for required minimum value
                ...
                // steps to calculate quick ratio
                double quick_assets = current_assets - inventory;
                double quick_ratio = (quick_assets / current_liabilities);
                quick_ratio = Math.round(quick_ratio * 100.0)/100.0;
                System.out.println(quick_ratio);
                String _result = (quick_ratio + " : 1");
                // steps to calculate result detail
                double result_detail = (quick_assets / current_liabilities);
                result_detail = Math.round(result_detail * 100.0)/100.0;
                System.out.println(result_detail);
                String _detail = "Based on the above result, company has Quick ratio of " + quick_ratio
                        + " : 1. It means $" + result_detail
                        + " of quick assets available to cover each $1 of short-term debt (current liabilities)." ;

                // Method used to copy the above declared strings from this activity and forward it to another activity
                Intent intent = new Intent(QuickRatioModule1.this, QuickRatioModule2.class);
    ...
    private void requiredCheck1() {
        AlertDialog.Builder builder = new AlertDialog.Builder(QuickRatioModule1.this);
    ...
    private void requiredCheck2() {
        AlertDialog.Builder builder = new AlertDialog.Builder(QuickRatioModule1.this);
        builder.setMessage("Inventory value must be at least 100 or greater value")
                .setCancelable(false)
                .setPositiveButton("OK", (dialog, i) -> {
                });
        AlertDialog alertDialog = builder.create();
    ...
    public void activity_reset(View view) {
        Intent intent = new Intent(this, QuickRatioModule1.class);
        startActivity(intent);
    }
}